...
model_ann <- train(formula, data = TrainSet,
                  method = "nnet", 
                  metric="RMSE",
...
gbmImp <- varImp(model_ann, scale = TRUE)
gbmImp

setwd(file.path(Working_path))

saveRDS(gbmImp,"RESULT/Model_varimportance_train_ANN")
#readRDS("SVM_Train_varimportance")

#png("TRAIN_varImportance_ANN.png")
tiff("RESULT/Model_varimportance_train_ANN.tiff", units="cm", width=8, height=8, res=600)
plot(gbmImp, top = 10)
dev.off()

# Do not change
saveRDS(model_ann, "RESULT/super_model_ann")
# Do not change

library(gmodels)
pred_train <-predict(model_ann, TrainSet[,-12])
pred_valid <-predict(model_ann, ValidSet[,-12])
...
saveRDS(perf_valid,"RESULT/ROC_Curve_valid_ANN")
#aa <- readRDS("Logreg_validation_ROC")
#png("TRAIN_roc_curve_train_LogReg.png")
tiff("RESULT/ROC_Curve_train_ANN.tiff", units="cm", width=8, height=8, res=600)
...
## Export accuracy
dput(perf.auc, "RESULT/Perf_AUC_train_ANN.txt")

#png("TRAIN_roc_curve_valid_ANN.png")
tiff("RESULT/ROC_Curve_valid_ANN.tiff", units="cm", width=8, height=8, res=600)
...
## Export accuracy  
dput(perf.auc_valid, "RESULT/Perf_AUC_ANN.txt")

# Predict raster with produced Super Model --------------------------------
## Apply to raster prediction
r1 <- raster::predict(raster_data, model_ann, progress="text")
plot(r1)

writeRaster(r1,"RESULT/Result_ANN.tif", overwrite=TRUE)
